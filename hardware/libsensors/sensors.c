...
#define AKM_DEVICE_NAME     "/dev/accel"
    ...
    *akm_fd = *p_fd = -1;
    while((de = readdir(dir))) {
        ...
        if (fd>=0) {
            ...
            else if (!strcmp(name, "proximity sensor")) {
                LOGV("using %s (name=%s)", devname, name);
                *p_fd = fd;
            }
            else if (!strcmp(name, "light sensor")) {
                LOGV("using %s (name=%s)", devname, name);
                *l_fd = fd;
            }
            else
...
static uint32_t read_akm_sensors_state(int fd, int flags)
{
    uint32_t sensors = 0;
...
static uint32_t read_cm_sensors_state(int fd, int flags)
{
    uint32_t sensors = 0;
    // read the actual value of all sensors
...
static uint32_t read_ls_sensors_state(int fd, int flags)
{
    uint32_t sensors = 0;
    // read the actual value of all sensors
...
static int control__set_delay(struct sensors_control_context_t *dev, int32_t ms)
{
    ...
    int delay = ms;
    if (!ioctl(dev->akmd_fd, KXSD9_IOC_SET_DELAY, &delay)) {
        return -errno;
    }
    return 0;
}
                                           ...
                                           struct input_event *event)
{
    ...
    if (event->type == EV_ABS) {
        ...
        case EVENT_TYPE_ACCEL_X:
            ...
            dev->sensors[ID_A].acceleration.x = -((event->value -2048 ) /81.9);//* CONVERT_A_X;
            break;
        case EVENT_TYPE_ACCEL_Y:
            new_sensors |= SENSORS_AKM_ACCELERATION;
            dev->sensors[ID_A].acceleration.y = -((event->value -2048 ) /81.9);// * CONVERT_A_Y;
            break;
        case EVENT_TYPE_ACCEL_Z:
            new_sensors |= SENSORS_AKM_ACCELERATION;
            dev->sensors[ID_A].acceleration.z = ((event->value -2048 ) /81.9);// * CONVERT_A_Z;
            break;
