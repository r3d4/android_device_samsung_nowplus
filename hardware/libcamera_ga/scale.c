...
int scale_process_preview(void* inBuffer, int inWidth, int inHeight, void* outBuffer, int outWidth, int outHeight, int rotation, int fmt, float zoom)
{
    ...
    double aspect_ratio;
    OMX_S32 inBufferLen;
    OMX_S32 outBufferLen;

    char ch;

    GPPToVPPInputFrameStatus*    pPrevIpFrameStatus;
    GPPToVPPOutputFrameStatus*   pPrevOpYUVFrameStatus;
    ...
    pPrevIpFrameStatus->ulCInOffset           = 0; /* offset of the C frame in the   *
                                                                                          ...
                                                                                          * is no C frame)                 */    
    /* crop */
    pPrevIpFrameStatus->ulInXstart            = (inWidth - (inHeight * 3 / 4)) / 2;
    pPrevIpFrameStatus->ulInXsize             = inHeight * 3 / 4;/*Default value for StdCompRoleTest */
    pPrevIpFrameStatus->ulInYstart            = 0;          
    pPrevIpFrameStatus->ulInYsize             = inHeight; /*Default value for StdCompRoleTest*/
    ...
    if( fmt ) {
        ...
        inBufferLen = inWidth * inHeight * 2;
        outBufferLen = (outWidth*outHeight) + ((outWidth*outHeight)>> 1);
    } else {
        pPrevOpYUVFrameStatus->ulCOutOffset   = 0;/*  Offset of the C frame in the buffer */
        inBufferLen = inWidth * inHeight * 2;
        outBufferLen = (outWidth*outHeight)*2;
    }
    ...
    pPrevIpFrameStatus->ulInWidth             = outWidth;
    pPrevIpFrameStatus->ulInHeight            = outHeight;    
    pPrevIpFrameStatus->ulCInOffset           = 0; /* offset of the C frame in the   *
                                                                                          * buffer (equal to zero if there *
                                                                                          * is no C frame)                 */    
    /* crop */
    pPrevIpFrameStatus->ulInXstart            = 0;
    pPrevIpFrameStatus->ulInXsize             = outWidth;/*Default value for StdCompRoleTest */
    pPrevIpFrameStatus->ulInYstart            = 0;          
    pPrevIpFrameStatus->ulInYsize             = outHeight; /*Default value for StdCompRoleTest*/
    ...
    pPrevIpFrameStatus->ulMirror              = OMX_FALSE;      
    pPrevIpFrameStatus->ulRGBRotation         = 0;    
    pPrevIpFrameStatus->ulYUVRotation         = 90;
    ...
    /*Init pComponentPrivate->pPrevOpYUVFrameStatus */
    pPrevOpYUVFrameStatus->ulOutWidth         = inHeight;
    pPrevOpYUVFrameStatus->ulOutHeight        = inWidth;
    if( fmt ) {
        pPrevOpYUVFrameStatus->ulCOutOffset   = inWidth*inHeight; /*  Offset of the C frame in the buffer */
        outBufferLen = outWidth * outHeight * 2;
        inBufferLen = (inWidth*inHeight) + ((inWidth*inHeight)>> 1);
    } else {
        pPrevOpYUVFrameStatus->ulCOutOffset   = 0;/*  Offset of the C frame in the buffer */
        outBufferLen = outWidth * outHeight * 2;
        inBufferLen = (inWidth*inHeight)*2;
...
int scale_process_capture(void* inBuffer, int inWidth, int inHeight, void* outBuffer, int outWidth, int outHeight, int rotation, int fmt, float zoom)
{
    ...
    pPrevOpYUVFrameStatus->ulOutHeight        = inWidth;
    if( fmt ) {
        pPrevOpYUVFrameStatus->ulCOutOffset   = inHeight*inWidth; /*  Offset of the C frame in the buffer */
        inBufferLen = outWidth * outHeight * 2;
        outBufferLen = (inHeight*inWidth) + ((inHeight*inWidth)>> 1);
    } else {
        pPrevOpYUVFrameStatus->ulCOutOffset   = 0;/*  Offset of the C frame in the buffer */
        inBufferLen = outWidth * outHeight * 2;
        outBufferLen = (inHeight*inWidth)*2;
    }
...
int scale_init(int inWidth, int inHeight, int outWidth, int outHeight, int inFmt, int outFmt)
{
    ...
    if( err != OMX_ErrorNone ) {
        LOGE("Fill_LCMLInitParams error = 0x%08x", err);
        return -1;
    }
...
int scale_deinit()
{
    ...
    if( err != OMX_ErrorNone ) {
        LOGE("LCML_ControlCodec(MMCodecControlStop) error=0x%08x\n", err);
        return -1;
    }
