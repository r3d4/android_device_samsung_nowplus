...
public:
    ...
    virtual size_t getInputBufferSize(
        uint32_t sampleRate, int format, int channelCount);
            ...
            const char *getFmOutputRouteFromDevice(uint32_t device);
            const char *getMax9877ModeFromDevice(uint32_t device);
            ...
            status_t setInputRoute(audio_source source, uint32_t device);
...
private:
    ...
    struct MixerConfig
    {
        const char *ctl;
        const char *val;
        int delay;    //delay after setting
    };


    status_t setOutputRoute( AudioHardware::RouteType path, uint32_t device);
